        ...
        return new Promise((resolve, reject) => {
            if (this.timestamp > new Date().getTime() - EXPIRE && this.account && this.token) {
                ...
                request(options, (err, resp) => {
                    if (err) {
                        reject(this.error(500, 'Internal Server Error'));
        ...
        return new Promise((resolve, reject) => {
            if (pipe && pipe.res) {
                ...
                if (_.isString(pipe.req)) {
                    ...
                    if (ops.method == 'POST') {
                        x = request.post(url, options, callback);
                    } else {
                        x = request.put(url, options, callback);
                    }
                    fs.createReadStream(pipe.req, { autoClose: true }).on('error', ()=>{
                        reject(this.error(500, 'Internal Server Error'));
                    }).pipe(x);
                } else {
